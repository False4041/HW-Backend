...
На рисунке 3 показаны результаты запросов через `curl`: первые четыре запроса вернули HTTP 200, а `/anything-else` - HTTP 404. Далее приведены ответы тех же маршрутов в браузере (рисунки 4-8).

![Рисунок 3 - Проверка маршрутов через curl](screenshots/curl.png)

*Рисунок 3 - Проверка маршрутов через curl*

![Рисунок 4 - GET / возвращает текстовый ответ](screenshots/root.png)

*Рисунок 4 - GET / возвращает текстовый ответ*

![Рисунок 5 - GET /api/matches возвращает два матча](screenshots/matches.png)

*Рисунок 5 - GET /api/matches возвращает два матча*

![Рисунок 6 - GET /api/teams возвращает четыре команды](screenshots/teams.png)

*Рисунок 6 - GET /api/teams возвращает четыре команды*

![Рисунок 7 - GET /api/matches/1 подтверждает получение ID](screenshots/param.png)

*Рисунок 7 - GET /api/matches/1 подтверждает получение ID*

![Рисунок 8 - JSON-ответ при обращении к неизвестному адресу](screenshots/not-found.png)

*Рисунок 8 - JSON-ответ при обращении к неизвестному адресу*

### 4.5 Логирование и перезапуск

На рисунке 9 показан журнал запросов. Для маршрутов `/`, `/api/matches`, `/api/teams` и `/api/matches/1` сервер вернул код 200, для `/anything-else` - код 404. Запрос `/favicon.ico` относится к значку страницы в браузере; отдельный маршрут для него не задан.

![Рисунок 9 - Журнал запросов сервера](screenshots/logs.png)

*Рисунок 9 - Журнал запросов сервера*
